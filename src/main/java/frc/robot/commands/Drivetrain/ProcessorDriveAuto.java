...
public class ProcessorDriveAuto extends Command {
  /** Creates a new Drive. */
  Drivetrain dt;
  double x;
  double y;
  ...
  public ProcessorDriveAuto(Drivetrain dt_imp, double x, double y, double rot, double max_speed) {
    this.dt = dt_imp;
    this.x = x;
    this.y = y;
    this.rot = rot;
  ...
  @Override
  public void execute() {
    double yaw = dt.get_yaw().getDegrees() % 360;
    ...
    rotation = -rotation_limiter.calculate(MathUtil.clamp(rot_pid.calculate(yaw, 180), -1, 1)) * Constants.dt.max_angular_speed;
