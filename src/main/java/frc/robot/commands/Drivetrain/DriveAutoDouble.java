...
public class DriveAutoDouble extends Command {
  /** Creates a new Drive. */
  Drivetrain dt;
  double x;
  double y;
  ...
  public DriveAutoDouble(Drivetrain dt_imp, double x, double y, double rot, double max_speed) {
    this.dt = dt_imp;
    this.x = x;
    this.y = y;
    this.rot = rot;
  ...
  public void execute() {
    ...
    rotation = -rotation_limiter.calculate(MathUtil.clamp(rot_pid.calculate(yaw, -90), -1, 1)) * Constants.dt.max_angular_speed;
